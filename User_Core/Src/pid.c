...
{
    pid->Setpoint = ReductionRatio * want / 360 - GIM6010[id].data_pos;    //在初始零点的基础上进行位置控制
}
...
void PID_PosLocCalc(PIDTypeDef *pid, int32_t feedbackpos,uint8_t id)//位置式
{
    float Now_Point,Now_Error,d_Error;
    Now_Point = (float )(feedbackpos - GIM6010[id].data_pos);
